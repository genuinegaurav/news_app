...
class IndiaScreen extends StatefulWidget {
  ...
  IndiaScreen({required this.isDarkMode});
...
class _IndiaScreenState extends State<IndiaScreen>
  ...
  Future<void> fetchIndiaNews() async {
    setState(() {
      isLoading = true;
    });
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load India news');
      }
    } catch (e) {
      setState(() {
        isLoading = false;
  ...
  Future<void> fetchMoreIndiaNews() async {
    setState(() {
      isLoadingMore = true;
    });
    ...
    try {
      final response = await http.get(Uri.parse(
          'https://news.google.com/topics/CAAqJQgKIh9DQkFTRVFvSUwyMHZNRE55YXpBU0JXVnVMVWRDS0FBUAE?hl=en-IN&gl=IN&ceid=IN%3Aen&page=$currentPage'));
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load more India news');
      }
    } catch (e) {
      setState(() {
        isLoadingMore = false;
